    ...
    <div class="content">
      ...
      <div class="projects">
        <div v-for="project in projects" :key="project.id" class="project-card">
          <img :src="project.image" alt="專案封面" />
          <h2>{{ project.title }}</h2>
          <p>{{ project.description }}</p>
          <a :href="project.link" target="_blank" class="view-button">查看專案</a>
        </div>
      </div>
    </div>
  </div>
...
const projects = ref([
  ...
  },
  {
    id: 3,
    title: "個人部落格網站",
    description: "使用 Vue + Markdown 建置的部落格系統。",
    image: "https://via.placeholder.com/300x150?text=Project+3",
    link: "https://你的blog-link"
  }
])
</script>
...
.portfolio-page {
  ...
  height: fit-content;
  min-height: 100vh;
  padding-bottom: 2rem; /* 給底部一點空間，不會太擠 */
}
